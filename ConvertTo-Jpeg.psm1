...
# foked by Beau
# https://github.com/beaukinstler/ConvertTo-Jpeg.git
...
function ConvertTo-Jpeg{
    ...
    Param (
        ...
        $FixExtensionIfJpeg
        ,        [Parameter(
            HelpMessage = "Embed old extension in the name before the '.jpg' extension")]
        [Switch]
        $EmpedOldExtension
    )
    ...
    Process
    {
        ...
        foreach ($file in $Files)
        {
            ...
            try
            {
                ...
                if($EmpedOldExtension.IsPresent){
                    $outputFileName = $inputFile.Name +  ".jpg"
                }
                else{
                    $oldExtension = ($inputFile.Name -split "\.")[-1]
                    $outputFileName = $inputFile.Name -replace ($oldExtension + "$"), "jpg"
                }
